"""
Numerical correctness tests: fused Triton kernels vs native PyTorch.
...
def test_fused_cross_entropy_matches_pytorch():
    ...
    # simulate some ignored tokens (e.g. padding)
    targets[::37] = -100
    ...
    assert torch.allclose(logits_ref.grad, logits_fused.grad, atol=1e-3, rtol=1e-3), \
        "gradient mismatch between reference and fused cross-entropy"
...
def test_fused_rmsnorm_matches_pytorch():
    ...
    assert torch.allclose(y_ref, y_fused, atol=1e-3, rtol=1e-3), "RMSNorm forward mismatch"

    grad_out = torch.randn_like(y_ref)
    y_ref.backward(grad_out)
    y_fused.backward(grad_out)

    assert torch.allclose(x_ref.grad, x_fused.grad, atol=1e-3, rtol=1e-3), "RMSNorm dX mismatch"
...
def test_fused_swiglu_matches_pytorch():
    ...
    assert torch.allclose(out_ref, out_fused, atol=1e-3, rtol=1e-3), "SwiGLU forward mismatch"

    grad_out = torch.randn_like(out_ref)
    out_ref.backward(grad_out)
    out_fused.backward(grad_out)

    assert torch.allclose(gate_ref.grad, gate_fused.grad, atol=1e-3, rtol=1e-3), "SwiGLU dGate mismatch"
    assert torch.allclose(up_ref.grad, up_fused.grad, atol=1e-3, rtol=1e-3), "SwiGLU dUp mismatch"
